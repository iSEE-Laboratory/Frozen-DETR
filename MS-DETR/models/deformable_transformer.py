...
class DeformableTransformer(nn.Module):
    def __init__(self, use_global_token, d_model=256, nhead=8,
        ...
        num_feature_levels_encoder = num_feature_levels + 1 if use_global_token else num_feature_levels

        encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels_encoder, nhead, enc_n_points)
        self.encoder = DeformableTransformerEncoder(encoder_layer, num_encoder_layers)

        decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, dec_n_points, use_ms_detr=use_ms_detr, use_aux_ffn=use_aux_ffn)
        self.decoder = DeformableTransformerDecoder(use_global_token, decoder_layer, num_decoder_layers, return_intermediate_dec, look_forward_twice=look_forward_twice, use_ms_detr=use_ms_detr)

        
        self.level_embed = nn.Parameter(torch.Tensor(num_feature_levels_encoder, d_model))
    ...
    def forward(self, srcs, masks, pos_embeds, query_embed=None, image_box=None, image_query=None, **kwargs):
        ...
        if image_query is not None:
            memory = memory[:, :-24*24]
            mask_flatten = mask_flatten[:, :-24*24]
        ...
        if self.two_stage:
            ...
            ###################################################################
            if image_query is not None:
                topk_coords_unact = torch.cat([topk_coords_unact, inverse_sigmoid(image_box)], dim=1)
            ###################################################################
            # end global query
            ###################################################################
        ...
        hs_o2o, hs_o2m, inter_references = self.decoder(tgt, reference_points, memory,
                                            spatial_shapes, level_start_index, valid_ratios, query_embed, mask_flatten, image_box=image_box, image_query=image_query, **kwargs)
...
class DeformableTransformerDecoder(nn.Module):
    def __init__(self, use_global_token, decoder_layer, num_layers, return_intermediate=False, look_forward_twice=False, use_ms_detr=False):
        super().__init__()
        ...
        if self.use_global_token:
            ...
            for i in range(num_layers):
                self.image_query_proj.append(nn.Linear(768, 256))
                self.image_query_norm.append(nn.LayerNorm(256))
    ...
    def forward(self, tgt, reference_points, src, src_spatial_shapes, src_level_start_index, src_valid_ratios,
                query_pos=None, src_padding_mask=None, image_box=None, image_query=None, **kwargs):
        output = tgt

        num_query = tgt.shape[1]
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if image_query is not None:
                ...
                output_o2m = output_o2m[:, :num_query]
...
def build_deforamble_transformer(args, use_global_token):
