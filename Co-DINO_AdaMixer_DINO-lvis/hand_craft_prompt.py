...
LVIS_CLASSES = [cls.replace('_', ' ') for cls in LVIS_CLASSES]

device = "cuda" if torch.cuda.is_available() else "cpu"
model, preprocess = clip.load("../ovd_resources/ViT-L-14-336px.pt", device=device)
model.eval()

with torch.no_grad():
    text_features_all = []
    for cetegory in LVIS_CLASSES:
        texts = [template.format(cetegory) for template in imagenet_templates] #format with class
        texts = clip.tokenize(texts).to(device)
        text_features = model.encode_text(texts)
        text_features = text_features / text_features.norm(dim=1, keepdim=True)
        text_features_all.append(torch.mean(text_features, dim=0))
    text_features_all = torch.stack(text_features_all, dim=0).float().cpu()


torch.save(text_features_all, 'lvis_manual_category_embeddings_ViT-L-14-336px.pt')
