...
class AdaMixerDecoderStage(BBoxHead):
    ...
    def __init__(self,
        ...
        self.use_class_embeddings = True if class_embedding is not None else False
        # over load the self.fc_cls in BBoxHead
        if self.loss_cls.use_sigmoid:
            if self.use_class_embeddings:
                self.fc_cls = nn.Linear(content_dim, global_token_dim)
                device = "cuda" if torch.cuda.is_available() else "cpu"
                category_embeddings = torch.load(class_embedding, 'cpu').float().to(device)
        ...
        if self.use_global_token:
            self.global_token_proj = nn.Linear(global_token_dim, content_dim)
            self.global_token_norm = nn.LayerNorm(content_dim)
    ...
    def xyxy2xyzr(self, bbox):
        ...
        z = ((wh).prod(-1, keepdim=True) + 1e-6).sqrt().log2()
        r = (wh[..., 1:2]/(wh[..., 0:1] + 1e-6) + 1e-6).log2()
    ...
    def forward(self,
        ...
        with torch.no_grad():
            ...
            if global_token is not None:
                roi_box_batched = torch.cat([roi_box_batched, image_box], dim=1)
            iof = bbox_overlaps(roi_box_batched, roi_box_batched, mode='iof')[
                :, None, :, :]
            iof = (iof + 1e-7).log()
            if global_token is not None:
                img_xyzr = self.xyxy2xyzr(image_box)
                pe_xyzr = torch.cat([query_xyzr, img_xyzr], dim=1)
            else:
                pe_xyzr = query_xyzr
            pe = position_embedding(pe_xyzr, query_content.size(-1) // 4)

        if global_token is not None:
            global_token = self.global_token_norm(self.global_token_proj(global_token))
            query_content = torch.cat([query_content, global_token], dim=1)
        ...
        if self.use_class_embeddings:
            cls_feat = self.fc_cls(cls_feat).view(N, n_query, -1)
            class_prompt = self.category_embeddings.unsqueeze(0).repeat(N, 1, 1)
            cls_score = torch.einsum('nqj,nmj->nqm', F.normalize(cls_feat, dim=-1), F.normalize(class_prompt, dim=-1)) * 50
        else:
