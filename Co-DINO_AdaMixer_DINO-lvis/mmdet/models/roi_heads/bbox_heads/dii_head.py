...
class DIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        if self.use_global_token:
            self.global_token_proj = nn.Linear(global_token_dim, in_channels)
            self.global_token_norm = nn.LayerNorm(in_channels)
    ...
    @auto_fp16()
    def forward(self, roi_feat, proposal_feat, image_box, image_query):
        ...
        if image_query is not None:
            image_query = self.global_token_norm(self.global_token_proj(image_query))
            proposal_feat = torch.cat([proposal_feat, image_query], dim=1)
...
class DIIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        self.attention_post = MultiheadAttention(in_channels, num_heads, dropout)
        self.attention_norm_post = build_norm_layer(dict(type='LN'), in_channels)[1]
    ...
    def init_weights(self):
        """Use xavier initialization for all weight parameter and set
        classification head bias as a specific value when use focal loss."""
        super(DIIIHead, self).init_weights()
        for p in self.parameters():
            if p.dim() > 1:
                nn.init.xavier_uniform_(p)
            else:
    ...
    def forward(self, roi_feat, proposal_feat):
        ...
        obj_feat = self.attention_norm_post(self.attention_post(obj_feat))
        obj_feat = obj_feat.permute(1, 0, 2)
