...
from mmdet.models.losses import QualityFocalLoss, FederatedQualityFocalLoss
...
class CoDINOHead(CoDeformDETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 num_global_token=1,
                 add_global_query_to_encoder=False,
        ...
        self.use_QualityFocalLoss = isinstance(self.loss_cls, QualityFocalLoss) or isinstance(self.loss_cls, FederatedQualityFocalLoss)
        ...
        if self.open_vocabulary:
            ...
            self.cls_branches = nn.ModuleList([nn.Linear(self.embed_dims, self.category_embeddings.shape[-1]) for _ in range(num_pred)])
    ...
    def init_denoising(self, dn_cfg):
        ...
        if self.use_global_token:
            self.dn_generator.num_queries += self.num_global_token

    def forward_train(self,
                      x, image_box, image_query, patch_token,
                      img_metas,
                      gt_bboxes,
                      gt_labels=None,
        ...
        outs = self(x, image_box, image_query, patch_token, img_metas, dn_label_query, dn_bbox_query, attn_mask)
    ...
    def forward_train_aux(self,
        ...
        outs = self.forward_aux(x[:-1], image_box, image_query, patch_token, img_metas, aux_targets, head_idx)
    ...
    def forward(self,
                mlvl_feats, image_box, image_query, patch_token,
                img_metas,
                dn_label_query=None,
                dn_bbox_query=None,
                attn_mask=None):
        ...
        if image_query is not None:
            image_box = bbox_xyxy_to_cxcywh(image_box)
        ...
        # begin global query
        ###################################################################
        if image_query is not None and self.add_global_query_to_encoder:
            mlvl_masks.append(mlvl_feats[0].new_zeros((batch_size, patch_token.shape[-2], patch_token.shape[-1])).to(torch.bool))
        ...
        for lvl in range(hs.shape[0]):
            ...
            if self.open_vocabulary:
                outputs_class = torch.einsum('bqd,cd->bqc', outputs_class, self.category_embeddings)
    ...
    def forward_aux(self, mlvl_feats, image_box, image_query, patch_token, img_metas, aux_targets, head_idx):
        """Forward function.
    ...
    def simple_test(self, feats, image_box, image_query, patch_token, img_metas, rescale=False):
        ...
        Returns:
            list[tuple[Tensor, Tensor]]: Each item in result_list is 2-tuple.
                ...
                with shape (n, ).
        """
        return self.simple_test_bboxes(feats, image_box, image_query, patch_token, img_metas, rescale=rescale)
    
    def simple_test_bboxes(self, feats, image_box, image_query, patch_token, img_metas, rescale=False):
        ...
        outs = self.forward(feats, image_box, image_query, patch_token, img_metas)
